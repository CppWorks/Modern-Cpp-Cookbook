...
inline bool is_positive(int const n) { return n > 0; }
...
class range_selector {
 ...
 public:
  explicit range_selector(int const start, int const end) : start_(start), end_(end) {}

  bool operator()(int const n) { return start_ <= n && n <= end_; }
};
...
class __lambda_name_2__ {
 ...
 public:
  explicit __lambda_name_2__(int const start, int const end) : start_(start), end_(end) {}
  ...
  bool operator()(int const n) const { return start_ <= n && n <= end_; }
};
...
void execute() {
...

  {
    ...
    auto end{10};
    auto inrange =
        std::count_if(std::begin(numbers), std::end(numbers), range_selector(start, end));
