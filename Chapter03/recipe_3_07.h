...
namespace recipe_3_07 {
  ...
  template <typename T, typename... Ts> T sum(T head, Ts... tail)
  {
    return head + sum(tail...);
  }

  template <typename F, typename T1, typename T2> auto fold(F&& f, T1 v1, T2 v2)
  {
    return f(v1, v2);
  }

  template <typename F, typename T, typename... Ts> auto fold(F&& f, T head, Ts... tail)
  {
    return f(head, fold(std::forward<F>(f), tail...));
  }

  template <typename T> bool is_fibonacci(T v1, T v2, T v3)
  {
    return v1 + v2 == v3;
  }
  /*
...
bool is_fibonacci(T v1, T v2, T v3, Ts... args)
{
  ...
  template <typename T> bool is_any(T value, T elem)
  {
    return value == elem;
  }

  template <typename T, typename... Ts> bool is_any(T value, T first, Ts... rest)
  {
