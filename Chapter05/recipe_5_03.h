...
class bitvector {
 ...
 public:
  ...
  inline size_t count() const { return std::count(bv.cbegin(), bv.cend(), true); }
  ...
  inline bitvector& remove(size_t const index) {
    if (index >= bv.size()) throw std::out_of_range("Index out of range");
    bv.erase(bv.begin() + index);
    return *this;
  }

  inline bitvector& set(bool const value = true) {
    for (size_t i = 0; i < bv.size(); ++i) bv[i] = value;
    return *this;
  }
