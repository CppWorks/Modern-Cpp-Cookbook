...
class OnTheLine : public Catch::MatcherBase<point3d> {
 ...
 public:
  OnTheLine(point3d const& p1, point3d const& p2) : p1(p1), p2(p2) {}
  ...
  virtual bool match(point3d const& p) const override {
    ...
    return Approx(rx).epsilon(0.01) == ry && Approx(ry).epsilon(0.01) == rz;
  }
...
TEST_CASE("assertions") {
  ...
  SECTION("matchers") {
    ...
    CHECK_THAT(text, Catch::Matchers::Contains("EXAMPLE", Catch::CaseSensitive::No));
    REQUIRE_THAT(text,
                 Catch::Matchers::StartsWith("this") && Catch::Matchers::Contains("an"));
  }
}
