...
void test_money() {
  ...
  std::cout << std::showbase << std::put_money(mon) << " or " << std::put_money(mon, true)
            << std::endl;

  for (auto const& locale : locale_names) {
    std::cout.imbue(std::locale(locale));
    std::cout << std::showbase << locale << ": " << std::put_money(mon) << " or "
              << std::put_money(mon, true) << std::endl;
  }
...
void write_money() {
  ...
  std::cout << std::put_money(mon) << std::endl;                         // 123.46
  std::cout << std::showbase << std::put_money(mon) << std::endl;        // £123.46
  std::cout << std::showbase << std::put_money(mon, true) << std::endl;  // GBP 123.46

  std::string smon = "12345.67";

  std::cout << std::put_money(smon) << std::endl;                         // 123.46
  std::cout << std::showbase << std::put_money(smon) << std::endl;        // £123.46
  std::cout << std::showbase << std::put_money(smon, true) << std::endl;  // GBP 123.46
